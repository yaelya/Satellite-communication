...
%   Copyright 2010-2017 The MathWorks, Inc.
    ...
    methods(Hidden)
        function this = ClassificationTree1(X,Y,W,modelParams,...
                dataSummary,classSummary,scoreTransform)
            if nargin~=7 || ischar(W)
                error(message('stats:ClassificationTree1:ClassificationTree1:DoNotUseConstructor'));
            end
    ...
    methods(Static,Hidden)
        function this = fit(X,Y,varargin)
            temp = ClassificationTree1.template(varargin{:});
            this = fit(temp,X,Y);
        end
    ...
    methods
        ...
        %   See also fitctree, ClassificationTree, PruneList, PruneAlpha.
            ...
            if ~ischar(crit)
                error(message('stats:ClassificationTree1:prune:CritNotChar'));
            end
            
            allowedVals = {'error' 'impurity'};
            tf = strncmpi(crit,allowedVals,length(crit));
            if sum(tf)~=1
                error(message('stats:ClassificationTree1:prune:BadCrit'));
            end
            ...
            if tf(2)
                if strcmpi(this.ModelParams.SplitCriterion,'twoing')
                    error(message('stats:ClassificationTree1:prune:ImpurityDisallowedForTwoing'));
                end
    ...
    methods(Hidden)
        ...
        function [err,seerr,nleaf,bestlevel] = cvLoss(this,varargin)                        
            ...
            if ~ischar(treesize) || ~(treesize(1)=='s' || treesize(1)=='m')
                error(message('stats:ClassificationTree1:cvLoss1:BadTreeSize'));
            end
            ...
            % Get loss
            [err,seerr] = ...
                classreg.learning.classif.CompactClassificationTree1.stratifiedLossWithSE(...
    ...
    methods(Static,Hidden)
        ...
        function [X,Y,C,WC,Wj,prior,cost,nonzeroClassNames,rowsused] = ...
            ...
            zeroprior = prior==0;
            if all(zeroprior)
                error(message('stats:ClassificationTree1:removeZeroPriorAndCost:ZeroPrior'));
            end
            ...
            if all(toremove)
                error(message('stats:ClassificationTree1:removeZeroPriorAndCost:ZeroPriorOrZeroCost'));
            end
        ...
        function [X,Y,W,dataSummary,classSummary,scoreTransform] = ...
            ...
            if isempty(allClassNames)
                error(message('stats:ClassificationTree1:prepareData:EmptyClassNames'));
            end
            ...
            if ~isfloat(X)
                error(message('stats:ClassificationTree1:prepareData:BadXType'));
            end
            ...
            [X,Y,W,userClassNames,nonzeroClassNames,dataSummary.RowsUsed] = ...
                ClassificationTree1.processClassNames(...
                X,Y,W,userClassNames,allClassNames,dataSummary.RowsUsed);

            % Remove missing values
            [X,Y,W,dataSummary.RowsUsed] = ClassificationTree1.removeMissingVals(...
            ...
            [X,Y,~,WC,Wj,prior,cost,nonzeroClassNames,dataSummary.RowsUsed] = ...
                ClassificationTree1.removeZeroPriorAndCost(...
        ...
        function this = loadobj(obj)
            if isa(obj.Impl,'classreg.learning.impl.CompactTreeImpl')
                ...
                this = ClassificationTree1(obj.X,obj.PrivY,obj.W,...
