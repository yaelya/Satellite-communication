classdef FitTemplate1 < classreg.learning.internal.DisallowVectorOps
...
%   Copyright 2010-2018 The MathWorks, Inc.
    ...
    methods(Static,Hidden)
        function temp = make(method,varargin)
            ...
            if ~ischar(method)
                error(message('stats:classreg:learning:FitTemplate1:make:BadArgs'));
            end
            ...
            if ~tfclass && ~tfreg
                error(message('stats:classreg:learning:FitTemplate1:make:UnknownMethod', method));
            end
            if     tfclass && tfreg
                ...
                if isempty(type) && ismember(method,classreg.learning.ensembleModels())
                    ...
                    elseif ~iscell(learners)
                        error(message('stats:classreg:learning:FitTemplate1:make:BadLearnerTemplates'));
                    end
                    L = numel(learners);
                    ...
                    for l=1:L
                        meth = learners{l};
                        if isa(meth,'classreg.learning.FitTemplate1')
                            meth = meth.Method;
                        end
                        isc = ismember(lower(meth),lower(namesclass));
                        isr = ismember(lower(meth),lower(namesreg));
                        if ~isc && ~isr
                            error(message('stats:classreg:learning:FitTemplate1:make:UnknownMethod', meth));
                        end
                        types(l) = isc - isr;
                    end
                    if     all(types==1)
            ...
            if ~isempty(usertype) && ~strcmp(usertype,type)
                error(message('stats:classreg:learning:FitTemplate1:make:UserTypeMismatch', method, usertype));
            end
            
            % Make template
            temp = classreg.learning.FitTemplate1(method,modelArgs);
        ...
        function temp = makeFromModelParams(modelParams,varargin)
            if ~isa(modelParams,'classreg.learning.modelparams.ModelParams')
                error(message('stats:classreg:learning:FitTemplate1:makeFromModelParams:BadModelParams'));
            end
            ...
            temp = classreg.learning.FitTemplate1(method,args);
        ...
        function catchType(varargin)
            ...
            if ~isempty(type)
                error(message('stats:classreg:learning:FitTemplate1:catchType:NonEmptyType'));
            end
        end
    ...
    methods(Access=protected)
        function this = FitTemplate1(method,modelArgs)
    ...
    methods(Hidden)
        ...
        function [varargout] = fit(this,X,Y,varargin)            
            ...
            if isempty(this.Type)
                error(message('stats:classreg:learning:FitTemplate1:fit:BadType', this.Method));
            end
            ...
            if ~isempty(this.CVPartitionSize) && numel(Y)~=this.CVPartitionSize
                error(message('stats:classreg:learning:FitTemplate1:fit:CVPartitionSizeMismatch',...
        ...
        function disp(this)
            ...
            if strcmp(this.Method,'ByBinaryRegr')
                ...
                if isempty(temp)
                    fprintf('%s\n',getString(message('stats:classreg:learning:FitTemplate1:FitTemplateForByBinaryRegr')));
                    if isLoose
                        fprintf('\n');
                    end
                else
            ...
            fprintf('%s %s%s.\n',getString(message('stats:classreg:learning:FitTemplate1:FitTemplateFor')),...
                type,this.Method);
        ...
        function this = fillIfNeeded(this,type)
            ...
            if ~isempty(this.Type)
                if ~strcmp(type,this.Type)
                    error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:IncompatibleType', this.Type));
                end
            end
            this.Type = type;
            ...
            if ~docv && ~isempty(modelParams)
                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:ModelParamsWithoutCV'));
            end
            ...
            % time
            if dobag && dosubspace
                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:BothBaggingAndSubspaceNotAllowed'));
            end
            
            % Disallow sampling observations and boosting by majority
            % undersampling at the same time
            if dobag && dorus
                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:BothBaggingAndRUSBoostNotAllowed'));
            end
            
            % Disallow subspace and boosting by majority undersampling at
            % the same time
            if dosubspace && dorus
                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:BothSubspaceAndRUSBoostNotAllowed'));
            end
            ...
            switch this.Method
                case classreg.learning.simpleModels()
                    if     dobag
                        error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotBagSimpleModel', this.Method));
                    elseif dosubspace
                        error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotSubspaceSimpleModel', this.Method));
                    elseif dorus
                        error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotRUSBoostSimpleModel', this.Method));
                    elseif docv
                        this.MakeModelParams = @classreg.learning.modelparams.EnsembleParams.make;
                        if isempty(modelParams)
                            learnerTemplate = ...
                                classreg.learning.FitTemplate1.make(this.Method,'type',this.Type,...
                                baseArgsForLearner{:},otherArgs{:});
                        else
                            learnerTemplate = ...
                                classreg.learning.FitTemplate1.makeFromModelParams(modelParams,...
                                baseArgsForLearner{:},otherArgs{:});                            
                        end
                ...
                case classreg.learning.ensembleModels()           
                    ...
                    if     docv
                        ...
                        if isempty(modelParams)
                            ...
                            if ~isempty(sampleArgs)
                                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotRedefineBagArgs'));
                            end
                            if ~isempty(subspaceArgs)
                                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotRedefineSubspaceArgs'));
                            end
                            if ~isempty(undersamplerArgs)
                                error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:CannotRedefineRUSBoostArgs'));
                            end
                            ensembleTemplate = ...
                                classreg.learning.FitTemplate1.makeFromModelParams(modelParams,...
                                baseArgsForLearner{:},otherArgs{:},'nprint','off');             
                    ...
                    elseif dosubspace
                        ...
                        if strcmp(this.Type,'classification')
                            this.MakeFitObject = @classreg.learning.classif.ClassificationEnsemble1;
                        else
                ...
                otherwise
                    error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:UnknownAlgorithm', this.Method));
            end
            ...
            if isempty(this.ModelParams) % Make from scratch
                ...
                for n=1:2:numel(baseArgs)
                    if ~ischar(baseArgs{n})
                        error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:BadBaseFitObjectParameterType'));
                    end
                    if ~any(strncmpi(baseArgs{n},this.AllowedBaseFitObjectArgs,length(baseArgs{n})))
                       error(message('stats:classreg:learning:FitTemplate1:fillIfNeeded:UnknownBaseFitObjectParameter', baseArgs{ n }));
                    end
                end
        ...
        function this = setInputArg(this,name,value)
            if ~ischar(name)
                error(message('stats:classreg:learning:FitTemplate1:setInputArg:ArgNameNotChar'));
            end
            props = properties(this.ModelParams);
            [tf,loc] = ismember(lower(name),lower(props));
            if ~tf
                error(message('stats:classreg:learning:FitTemplate1:setInputArg:ArgNameNotFound', name));
            end
            foundprop = props{loc};
        ...
        function out = getInputArg(this,name)
            if ~ischar(name)
                error(message('stats:classreg:learning:FitTemplate1:getInputArg:ArgNameNotChar'));
            end
            props = properties(this.ModelParams);
            [tf,loc] = ismember(lower(name),lower(props));
            if ~tf
                error(message('stats:classreg:learning:FitTemplate1:getInputArg:ArgNameNotFound', name));
            end
            foundprop = props{loc};
        ...
        function tf = isemptyInputArg(this,name)
            if ~ischar(name)
                error(message('stats:classreg:learning:FitTemplate1:isemptyInputArg:ArgNameNotChar'));
            end
            props = properties(this.ModelParams);
            [tf,loc] = ismember(lower(name),lower(props));
            if ~tf
                error(message('stats:classreg:learning:FitTemplate1:isemptyInputArg:ArgNameNotFound', name));
            end
            foundprop = props{loc};
        ...
        function this = setBaseArg(this,name,value)
            if ~ischar(name)
                error(message('stats:classreg:learning:FitTemplate1:setBaseArg:ArgNameNotChar'));
            end
            f = @(x) strcmpi(name,x);
            ...
            if isempty(loc) % add this property
                ...
                if loc(1)+1>length(this.BaseFitObjectArgs)
                    error(message('stats:classreg:learning:FitTemplate1:setBaseArg:MissingValueInPropertyList', name));
                end
...
% Type - classification or regression?
if ~ischar(type)
    error(message('stats:classreg:learning:FitTemplate1:gettype:BadType'));
end
loc = find(strncmpi(type,{'classification' 'regression'},length(type)));
if isempty(loc)
    error(message('stats:classreg:learning:FitTemplate1:gettype:UnknownType'));
end
if loc==1
    type = 'classification';
else
