function get_titanic_train(foldin, file, foldout, alreadydone)
% get_titanic_train('data', 'train', 'datamat', 0)
...
for i=1:size(lost,2)
    ...
    x(2,i)=str2double(data(lost(2,i)+1:lost(3,i)-1)); % Pclass (1 = 1st; 2 = 2nd; 3 = 3rd)
    % Name => don't care
    x(3,i)=strcmp(data(lost(5,i)+1:lost(6,i)-1),'female'); % Sex
    x(4,i)=str2double(data(lost(6,i)+1:lost(7,i)-1)); % Age
    x(5,i)=str2double(data(lost(7,i)+1:lost(8,i)-1)); % SibSp Number of Siblings/Spouses Aboard
    x(6,i)=str2double(data(lost(8,i)+1:lost(9,i)-1)); % Parch Number of Parents/Children Aboard
    % Ticket => don't care
    x(7,i)=str2double(data(lost(10,i)+1:lost(11,i)-1)); % Passenger Fare
    % t11(i)= % Cabin => sure you don't care?
    switch data(lost(12,i)+1);
        % Port of Embarkation 
