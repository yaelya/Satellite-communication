...
rusTree = fitcensemble(data(istrain,1:3),Y(istrain),'Method','RUSBoost', ...
    'NumLearningCycles',300,'Learners',t,'LearnRate',0.1,'nprint',301);
figure;
plot(loss(rusTree,data(istest,1:3),Y(istest),'mode','cumulative'));
grid on;
xlabel('Number of trees');
ylabel('Test classification error');

Yfit = predict(rusTree,data(istrain,1:3));
...
L=loss(cmpctRus,data(istrain,1:3),Y(istrain));
